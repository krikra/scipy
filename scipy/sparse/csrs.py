"""Compressed Sparse Row-Symmetric matrix format"""
...
class csrs_matrix(csr_matrix, IndexMixin):
    ...
    def transpose(self, axes=None, copy=False):
        ...
        if copy:
            return csrs_matrix((self.data, self.indices,
                                self.indptr), shape=self.shape, copy=True)
        else:
            return self
    ...
    def tocsr(self, copy=False):
        ...
        indices = np.empty(self.nnz * 2 - self.shape[0], dtype=idx_dtype)
        data = np.empty(self.nnz * 2 - self.shape[0], dtype=upcast(self.dtype))

        csrs_tocsr(self.shape[0], self.shape[1],
    ...
    def _get_row_slice(self, i, cslice):
        ...
        if stride == 1:
            ...
            row_indptr, row_indices, row_data = get_csrs_submatrix(
...
def isspmatrix_csrs(x):
    return isinstance(x, csrs_matrix)
